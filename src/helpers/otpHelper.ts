...
async function emailOtp(
    ...
    console.log(`OTP Helper : Otp ${otp}  sent to ${userEmail}`);
}
...
function isOtpExpired(otpCreationTime: Date): boolean {
    ...
    console.log(
        `OTP Helper : Time (in ms) since OTP creation -> ${timeDifference}`
    );
...
async function verifyOtp(
    ...
    const searchOtpResult: IOtp | null = await OtpModel.findOne({
        emailId: userEmail,
        otpType: otpType,
    });
...
export { emailOtp, verifyOtp, resendOtp, sendPasswordResetEmail };
