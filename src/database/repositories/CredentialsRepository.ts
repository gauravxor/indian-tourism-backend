...
class CredentialsRepository {
    static async searchCredentials(userId: string): Promise<ICredentials | null> {
        logger.info(`Searching credentials for userId: ${userId}`);
        const searchResult = await CredentialsModel.findOne({ userId: userId });
        return searchResult;
    }

    static async updatePassword(email: string, passwordHash: string): Promise<ICredentials | null> {
