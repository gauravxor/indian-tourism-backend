...
const updateLocationController = async (req, res, next) => {
	...
	try{
		...
		const updatedLocation = {
			name: name ? name : locationData.name,
			description: description ? description : locationData.description,
			address: address ? address : locationData.address,
			city: city ? city : locationData.city,
			state: state ? state : locationData.state,
			country: country ? country : locationData.country,
			pincode: pincode ? pincode : locationData.pincode,
			capacity: capacity ? capacity : locationData.capacity,
			ticketPrice: ticketPrice ? ticketPrice : locationData.ticketPrice
		};
